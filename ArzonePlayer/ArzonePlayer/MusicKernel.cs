    ...
    public class MusicKernel : IDisposable
    {
        ...
        public int PlaySpeed
        {
            set
            {
                if (stream != 0)
                {
                    Bass.BASS_ChannelSlideAttribute(stream, BASSAttribute.BASS_ATTRIB_FREQ, value, 0);
                }
            }
        }
        ...
        /// <summary>
        /// 获取FFT采样数据，返回1024个浮点采样数据
        /// </summary>
        /// <returns></returns>
        public float[] Get1024FFTData()
        {
        ...
        /// <summary>
        /// 获取FFT采样数据，返回2048个浮点采样数据
        /// </summary>
        /// <returns></returns>
        public float[] Get2048FFTData()
        {
        ...
        /// <summary>
        /// 获取FFT采样数据，返回4096个浮点采样数据
        /// </summary>
        /// <returns></returns>
        public float[] Get4096FFTData()
        {
            float[] fft = new float[4096];
            Bass.BASS_ChannelGetData(stream, fft, (int)BASSData.BASS_DATA_FFT8192);
            return fft;
        }
        /// <summary>
        /// 获取音频设备信息
